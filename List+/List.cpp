...
{
	//const迭代器
	list<int>::const_iterator it = lt.begin();     //lt是const，const不能调用非const成员
	while (it != lt.end())
	{
		//*it = 10;   //加该句之后可能被修改，因此前面要用const修饰t& operator*()，const t& operator*();但是这样之后所有的迭代器都不能再修改了——>更改模板类，.h文件的18,22,42-51，99,100行  
		cout << *it << " ";
		++it;
	}
	cout << endl;
...
void Test_List3()
{
	...
	while (it != l.end())
	{
		if (*it % 2 == 0)
		{
			it = l.Erase(it);   //Erase返回的是下一个位置的迭代器，再重新赋值给it，让它继续往后走
		}
		else
		{
			++it;
